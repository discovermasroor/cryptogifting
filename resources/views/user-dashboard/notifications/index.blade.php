@include('user-dashboard.head')
    <title>Notifications List | Dashboard</title>
    <body>
            ...
            <div class="main-content">
                <div class="overlay">
                    <div class="main-heading-box flex">
                        ...
                        <a class="btn blue-button" href="{{route('MarkAllSeen')}}">Mark all as Read</a>
                    </div>
                    <div class="notifications-wrapper">
                        <?php foreach ($notifications as $key => $value) { ?>
                            <a href="<?php if ($value->notification->event_id) echo route('EventPreview', [$value->notification->event_id]); else echo '#'; ?>">
    ...
    <script>
        jQuery(document).ready(function () {
            $('.dot-icon').on('click', function (e) {
                ...
                $.ajax({
                    url: '{{ route("ReadNotificationUser") }}',
                    type: 'POST',
                    data: formData,
                    dataType: 'JSON',
